...
import { ArrowLeft, Star, Plus, Minus, Trophy, Search } from "lucide-react";
...
const TayoPoints = () => {
  ...
  const fetchChildrenWithPoints = async () => {
    try {
      ...
      // Calculate total points for each child
      const childrenWithPoints = (childrenData || []).map(child => {
        const childTransactions = (transactionsData || []).filter(t => t.child_id === child.id);
        const totalPoints = childTransactions.reduce((sum, t) => sum + t.points, 0);
        ...
        <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{t('tayo.subtitle')}</p>
            </div>
          </div>
          <LanguageSwitcher />
        </div>
